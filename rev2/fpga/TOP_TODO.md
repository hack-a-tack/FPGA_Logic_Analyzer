...
8. **Post-route Fmax / WNS** — Rev1 baseline was ~60.26 MHz, WNS +4.238 ns. PARTIALLY
   DONE 2026-08-20/21 — full Synplify + P&R now runs and produces `top_timing.rpt`.
   However, the numbers it reports are measured against the wrong target: the SDC's
   `clk52` constraint (52.8 MHz / 18.939 ns, `rev2/fpga/constraints/synopsys_design_constraints.sdc`)
   does not bind inside the Lattice P&R chain (`sbtplacer`/`sbrouter`/`sbtimer`) — every
   one of them logs `Info-1409: Inferred clock at E1.HF_OSC/CLKHF` and falls back to an
   auto-inferred 48 MHz default instead. Confirmed via a diagnostic run (2026-08-21)
   trying `get_nets{E1.o_clk}`, `get_nets{CLK}`, `get_pins{E1.HF_OSC/CLKHF}`, and
   `get_pins{E1/HF_OSC/CLKHF}` side by side: every valid net-based target Synplify
   accepts gets auto-retargeted, internally, to the identical downstream pin reference
   `get_pins{E1.HF_OSC/CLKHF}` — and that exact string is what the Lattice tools fail to
   bind, regardless of which source-level target produced it. This is not fixable by
   editing the `.sdc` text; the failing string is generated by Synplify's own
   net-to-pin retargeting, not authored there. Synplify's own pre-route estimate (its
   `fpga_mapper` stage timer, which does bind `clk52` correctly) is the best currently
   available number against the real 52.8 MHz target: ~-2.15 to -2.28 ns slack at
   ~47 MHz, depending on the exact RTL revision — notably close to, not wildly off
   from, the post-route numbers measured against the wrong 48 MHz target. Next step:
